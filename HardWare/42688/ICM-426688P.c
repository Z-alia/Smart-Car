...
void ICM42688P_BurstRead_Internal(uint8_t reg_address, uint8_t *rxdata, uint8_t length)
{
    ...
    // 发送寄存器地址（带读标志）
    HAL_SPI_Transmit(&hspi4, &tx_addr, 1, HAL_MAX_DELAY);
    
    // 读取数据
    if (length > 0)
    {
        HAL_SPI_Receive(&hspi4, rxdata, length, HAL_MAX_DELAY);
    }
...
uint8_t ICM42688P_Init(void)
{
    ...
    if (whoami == 0x47)  // ✅ 修正：WHO_AM_I正确时才配置
    {
        // 初始化配置结构
        ...
        // 配置陀螺仪
        ICM42688P_ConfigGyro(&config, 0x01, 0x07, 0x02, 0x01);
        
        // 配置加速度计
        ICM42688P_ConfigAccel(&config, 0x02, 0x07, 0x02, 0x01);
		
		//配置零偏移量
		ICM42688P_ConfigGyroOffset(&config,0,0,3);
		
//		// 配置陀螺仪抗混叠
//		ICM42688P_ConfigGyroAAF(&config, 0,
        
        // 配置时钟
        ICM42688P_ConfigClock(&config, 0x01, 0x00, 0x00);
...
void ICM42688P_Clock_Config(void)
{
    ...
    uint8_t config = 0x06;//禁用p9
    ICM42688P_WriteRegister(0x7b, &config, 1);//pin9功能
    ICM42688P_Bank_Select(0);
    config = 0x95;
    ICM42688P_WriteRegister(0x4d, &config, 1);//时钟配置
}
